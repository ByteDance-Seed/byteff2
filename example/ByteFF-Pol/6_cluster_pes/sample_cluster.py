...
all_comps = {
    "n1": {"EtCl": 3},
    "n2": {"BZ": 3},
    "n3": {"EtCl": 1, "EtSH": 1, "NOM": 1},
    "n4": {"An": 2, "NOM": 1},
    "n5": {"ACE": 1, "ACN": 1, "BZ": 1, "HAC": 1},
    "i1": {"EC": 3, "PF6": 1},
    "i2": {"DMC": 4, "LI": 1},
    "i3": {"EC": 2, "EMC": 2, "LI": 1},
    "i4": {"EC": 3, "LI": 1, "PF6": 1},
...
def main(args):
    ...
    for cluster_name, cluster_comps in all_comps.items():
        ...
        for ts in u.trajectory[2000::10]:
            for _ in range(100):
                mols = sample_cluster(
                    ts.positions, ts.dimensions[:3], cutoff, atom_rec, res_rec, cluster_comps, smiles_map
                )
                if mols is None:
                    continue
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--md_samples_dir", type=str, default="./data/md_samples", help="where to save sampled clusters"
    )
