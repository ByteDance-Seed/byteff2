...
def gen_dimer_lines(atoms, net_charge):
    ...
    for fragment_atoms, fragment_charge in zip(atoms, net_charge):
        lines.append("--\n")
        lines.append(f"{fragment_charge} 1\n")
        for symb, coord in zip(fragment_atoms.symbols, fragment_atoms.positions):
