...
def main(mapped_smiles, mol_name):
    ...
    with open(qcin, "w") as f:
        f.write(qc_input)
    command = f"QCSCRATCH=./ geometric-optimize --nt 8 --converge set GAU --engine qchem {qcin}"
    ...
    with h5py.File(h5_file, "r") as f:
        ...
        atoms = ase.Atoms(
            numbers=last_job["structure"]["nuclei"][()], positions=last_job["structure"]["coordinates"][()] * Bohr
        )
