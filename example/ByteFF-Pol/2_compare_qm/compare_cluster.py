...
def get_ff_energy(model, _mols, conformer=0):
    ...
    for key, value in ff_results.items():
        ff_results[key] = value.detach().tolist()
    ff_results["ELEC_PAULI"] = np.array(ff_results["ELEC"]) + np.array(ff_results["PAULI"])
    ff_results["ELEC_PAULI_DISP"] = (
        np.array(ff_results["ELEC"]) + np.array(ff_results["PAULI"]) + np.array(ff_results["DISP"])
    )
    return ff_results
...
if __name__ == "__main__":
    ...
    for dirname in sorted(os.listdir(src)):
        ...
        mols = [Molecule(fp) for fp in files if "optim" not in fp]
        ...
        with temporary_cd(save_dd):
            new_mols = hybridff_ase_opt(
                mols, model=trained_model, conformer=-1, save_trj=True, position_restraint=position_restraint
            )
            rmsd = 0.0
