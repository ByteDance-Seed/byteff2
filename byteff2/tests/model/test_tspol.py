...
from byteff2.bytemol.utils import get_data_file_path, setup_default_logging
    ...
    for ib, top in enumerate(tops):
        if pme:
            calculator = AmoebaCalculator(
                ...
                unit_cell=[20.0, 20.0, 20.0],
                cutoff=1.0,
            )
        else:
            calculator = AmoebaCalculator(
                top, separate_terms=True, nonbonded_params=nonbonded_params, platform_name=device.upper()
            )
...
def summary_preds(preds, cluster):
    ...
    model_coul_e = (
        ff_parameters["Exp6Pol.perm_chg_energy" + surfix] + ff_parameters["Exp6Pol.induction_energy" + surfix]
    )
    model_coul_f = (
        ff_parameters["Exp6Pol.perm_chg_forces" + surfix] + ff_parameters["Exp6Pol.induction_forces" + surfix]
    )
    ...
    for j, (p, t) in enumerate(zip(preds, target)):
        ...
        if batch_size is not None:
            for i in range(batch_size):
                assert torch.allclose(p[shape[j] * i : shape[j] * (i + 1)], t, rtol=rtol, atol=atol), (
                    (p[shape[j] * i : shape[j] * (i + 1)] - t).abs().max()
                )
        else:
...
def check_tspol_dimer(model, device, pme=False):
    ...
    data = ClusterData(
        ...
        max_n_confs=5,
        confdata=dict(
            coords=np.concatenate([np.array([mol.conformers[i].coords for i in range(nconfs)]) for mol in mols], axis=1)
        ),
        moldata=dict(
            charge=np.concatenate([mol.get_conf_prop("charges") for mol in mols]),
            volume=np.concatenate([mol.get_conf_prop("volumes") for mol in mols]),
        ),  # A**3
    )
    data = data.to(device)
