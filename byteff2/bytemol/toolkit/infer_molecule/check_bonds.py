...
bl_data = pd.read_csv(get_data_file_path("bond_length_ref.csv", "byteff2.bytemol.toolkit.infer_molecule"))
bond_length_ref = {}
...
def check_new_bonds(mol: Molecule, conf_id=0, vdwradii=None, fudge_factor=0.55, lower_bound=0.1) -> set[tuple[int]]:
    ...
    bonds = guess_bonds(
        u.atoms, u.atoms.positions, vdwradii=vdwradii, fudge_factor=fudge_factor, lower_bound=lower_bound
    )
