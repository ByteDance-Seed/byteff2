...
from byteff2.bytemol.core import Conformer, Molecule
from byteff2.bytemol.core.conformer import is_energy_key, is_force_key
from byteff2.bytemol.core.moleculegraph import MoleculeGraph
import byteff2.bytemol.toolkit.asetool.geometry_numpy as kernel
...
class HarmonicRestraints:
    ...
    def get_position_restraints_energy_forces(self, coords: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        grad_displacement = np.where(
            (delta_distance > 0)[..., np.newaxis],
            -force_constant[..., np.newaxis]
            * delta_distance[..., np.newaxis]
            * displacement
            / distance[..., np.newaxis],
            0,
        )
    ...
    def get_dihedral_restraints_energy_forces(self, coords: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        dihedral, f1, f2, f3, f4 = kernel.get_dihedral_angle_vec(
            coords[np.newaxis, np.newaxis, :, :], atomidx[np.newaxis, :, :]
        )  # rad
...
class BaseCalculator(Calculator, abc.ABC):
    ...
    @abc.abstractmethod
    def _calculate_without_restraint(
        self, coords: np.ndarray, calc_force: bool = True
    ...
    def set_restraints(
        ...
        force_constant: force constant of restraints, recommand 1 kcal/mol/A2 for position restraints and
            0.1 kcal/mol/rad2 for dihedral restraints
        restraint_type:
        ...
        if restraint_type == "position":
            restraint_torsions = []
            self.set_position_restraints(
                atomidx=np.arange(molgraph.natoms),
