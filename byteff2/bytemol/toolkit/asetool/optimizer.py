...
# from byteff2.bytemol.core import Topology
from byteff2.bytemol.toolkit.asetool.optimizer_config import OptimizerConfig
    ...
    if config.get_optimizer_type() == "sella":
        ...
        if not no_reaction:
            ...
            for i, j in bonds:
                ...
                cons.fix_bond(
                    (i, j), target=no_reaction_scale * ase_frame.get_distance(i, j), comparator="lt", replace_ok=False
                )
            internals = Internals(ase_frame, cons=cons)  # all constraints will be automatically added to IC
        ...
        logger.info(
            "constructed %s bonds, %s angles, %s dihedrals in sella",
            internals.nbonds,
            internals.nangles,
