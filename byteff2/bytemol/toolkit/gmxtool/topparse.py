...
class RecordPair(RecordText):
    ...
    def get_params(
        self, td: TopoDefaults, vws: list[Tuple[float, float]], charges: list[float]
...
class RecordDihedral(RecordText):
    def _check(self):
        if self.funct in (
            DihedralTypeEnum.PROPER,
            DihedralTypeEnum.PERIODIC_IMPROPER,
    ...
    def _str_impl(self):
        ...
        if self.funct == DihedralTypeEnum.RYCKAERT_BELLEMANS:
            return (
                line
                + f"{self.c0:>16.6f}{self.c1:>16.6f}{self.c2:>16.6f}{self.c3:>16.6f}{self.c4:>16.6f}{self.c5:>16.6f}"
            )
        elif self.funct in (
            DihedralTypeEnum.PROPER,
            DihedralTypeEnum.PERIODIC_IMPROPER,
    ...
    @property
    def phi(self):
        assert self.funct in (
            DihedralTypeEnum.PROPER,
            DihedralTypeEnum.PERIODIC_IMPROPER,
            DihedralTypeEnum.MULTIPLE_PROPER,
            DihedralTypeEnum.IMPROPER,
        )
        return self.c0
...
class RecordVirtualSite2(RecordText):
    def _init(
        self, av: int = None, ai: int = None, aj: int = None, funct: VirtualSite2Enum = None, c: float = None, **kwargs
    ):
        super()._init(**kwargs)
...
class RecordVirtualSite4(RecordText):
    ...
    def _str_impl(self):
        return (
            f"{self.av:>10}{self.ai:>10}{self.aj:>10}{self.ak:>10}{self.al:>10}{self.funct:>6}"
            f"{self.c0:>16.6f}{self.c1:>16.6f}{self.c2:>16.6f}"
        )
...
class RecordDihedralRestraints(RecordText):
    ...
    def _str_impl(self):
        line = (
            f"{self.ai:>10}{self.aj:>10}{self.ak:>10}{self.al:>10}{self.funct:>6}"
            f"{self.phi:>16.6f}{self.dphi:>16.6f}{self.fc:>16.6f}"
        )
        return line
...
class RecordPositionRestraints(RecordText):
    def _check(self):
        assert self.funct in {1, 2}

    def _init(
        self,
        ai: int = None,
        funct: int = None,
        x: Union[int, float] = None,  # x is int or float
        y: Union[int, float] = None,  # y is int or float
        z: Union[int, float] = None,  # z is int or float
        comment: str = None,
        **kwargs,
    ):
        """
        reference: https://manual.gromacs.org/documentation/current/reference-manual/topologies/topology-file-formats.html
        if function type is 1, the parameters x, y, z are used, which are direction, radius, and force constant
        if function type is 2, the parameters x, y, z means kx, ky, ky
        """
        super()._init(**kwargs)
        self.ai, self.funct = ai, funct
    ...
    def _parse_line(self):
        fields = self._fields
        self.ai, self.funct, self.x, self.y, self.z = (
            int(fields[0]),
            int(fields[1]),
            float(fields[2]),
            float(fields[3]),
    ...
    def cmp_key(self):
        return (self.ai, self.funct, self.x, self.y, self.z)

    def annotate(self) -> str:
        line = ""
        if self.funct == 1:
            line = ";       ai        funct       fx       fy       fz"
        elif self.funct == 2:
            line = ";       ai        funct        g        r        k"
        return line
    ...
    def _str_impl(self):
        line = ""
        if self.funct == 1:
            line = f"{self.ai:>10}{self.funct:>10}{int(self.x):>10}{int(self.y):>10}{int(self.z):>10}"
        elif self.funct == 2:
            line = f"{self.ai:>10}{self.funct:>10}{int(self.x):>10}{float(self.y):>10}{float(self.z):>10}"
        return line
        ...
        with open(path) as f:
            for raw in f:
                ...
                if text == "":
                    ...
                    r = RecordInclude.from_text(
                        dirname=dirname, text=text
                    )  # filter the comment after #include "file.path"
                    fs.all.append(r)
...
class TopoMolecule:
    ...
    def update_charge(
        self, partial_charges: Iterable[float] = None, symm: dict[int, list[int]] = None, to_round: bool = False
    ):
        if to_round:
            assert self.allow_round_diff is not None
            assert self.allow_round_diff >= 0.0
            pchgs = self.round_list_sum_to_int(
                self.get_charges() if partial_charges is None else partial_charges, symm, self.allow_round_diff
            )
    ...
    def from_records(
        ...
        *,
        ...
        for r in records:
            # NOTE: skip [ intermolecular_interactions ] when parsing top files to avoid counting bonds, angles,
            # and dihedrals included in this section as terms of a particular molecule.
            if isinstance(r, RecordMoleculeType) or (
                isinstance(r, RecordSection) and r.section == "intermolecular_interactions"
            ):
        ...
        *,
        ...
        for idx, r in enumerate(records):
            if isinstance(r, RecordMoleculeType):
                m = TopoMolecule.from_records(
                    system_uuid=tfs.uuid,
                    records=records[idx:],
                    sort_idx=sort_idx,
                    round_on=round_on,
                    allow_round_diff=allow_round_diff,
