...
from byteff2.bytemol.core import rkutil
from byteff2.bytemol.core.molecule import Molecule
from byteff2.bytemol.core.rkutil.match_and_map import find_mapped_smarts_matches
from byteff2.bytemol.core.topology import Topology
...
class Atom:
    ...
    def __init__(self, atom: Chem.Atom = None, idx: int = None):
        ...
        if atom is None:
            ...
            self.hybridization = hyb_map.get(atom.GetHybridization(), Hybridization.OTHER)
...
class MoleculeGraph:
    ...
    def __init__(
        self, mol: Molecule = None, coords_conf_id: int = None, aromaticity: str = "rdkit", max_include_ring: int = 10
    ):
        """
        Parameters
        ----------
        ...
        if mol is not None:
            ...
            self._create_from_rkmol(max_include_ring)
    ...
    def _create_from_rkmol(self, max_include_ring):
        """
        ...
        self.topo = Topology(bonds=bonds, bond_ids=bond_ids, max_include_ring=max_include_ring)
    ...
    def get_bond_with_atoms(self) -> List[Tuple[Bond, Atom, Atom]]:
        return [
            (self._bond_by_idx[data["idx"]], self._atom_by_idx[a1], self._atom_by_idx[a2])
    ...
    def get_impropers(self) -> List[Tuple[int]]:
        atomsets = set()
        for atom in self.get_atoms():
            if atom.atomic_number in [6, 7] and atom.hybridization == Hybridization.SP2:
                idx = atom.idx
    ...
    def get_nonbonded14_pairs(self) -> list[tuple]:
    ...
    def get_nonring_rotatable_propers(self) -> List[Tuple[int]]:
        ...
        propers = [proper for proper in propers if proper not in linear]
        return propers
