...
def find_mapped_smarts_matches(
    rkmol: Chem.Mol,
    ...
    ** adapted from openff.toolkit.utils.rdkit_wrapper.py **
    ...
    if match_resonance:
        ...
        full_matches = suppl.GetSubstructMatches(
            qmol, uniquify=False, maxMatches=max_matches, useChirality=use_chirality
        )
    else:
        full_matches = rkmol.GetSubstructMatches(
            qmol, uniquify=False, maxMatches=max_matches, useChirality=use_chirality
        )
...
def find_indices_mapping_between_mols(
    rkmol0: Chem.Mol, rkmol1: Chem.Mol, verbose: bool = False, ring_matches_ring_only: bool = True, **kwargs
) -> Dict[int, int]:
    """return the indices mapping from rkmol1 to rkmol0."""
    result = rdFMCS.FindMCS(
        [rkmol0, rkmol1], timeout=5, verbose=verbose, ringMatchesRingOnly=ring_matches_ring_only, **kwargs
    )
