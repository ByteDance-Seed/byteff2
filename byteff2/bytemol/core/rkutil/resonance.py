...
from byteff2.bytemol.core.rkutil.information import get_nnz_formal_charges, get_sum_absolute_formal_charges
from byteff2.bytemol.core.rkutil.match_and_map import get_smiles
from byteff2.bytemol.core.rkutil.sanitize import sanitize_rkmol
...
def get_resonance_structures(mol: Chem.Mol, flags: int = 0, filter_by_formal_charges: bool = True) -> List[Chem.Mol]:
    ...
    prefiltered = []
    for m in resMols:
        # rdkit 2024 could return None in the list here
        if m is None:
            continue
    ...
    for resonance_mol in resMols:
        count = get_nnz_formal_charges(resonance_mol)
        filtered[count].append(resonance_mol)

    # then filter by total formal charges
    min_count = min(filtered.keys())
    filtered2 = defaultdict(list)
    for resonance_mol in filtered[min_count]:
        count = get_sum_absolute_formal_charges(resonance_mol)
        filtered2[count].append(resonance_mol)
