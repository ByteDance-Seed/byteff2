...
# The file is modified from https://github.com/pyg-team/pytorch_geometric/blob/master/torch_geometric/nn/models/basic_gnn.py
...
class GINE(BasicGNN):
    ...
    def init_conv(self, in_channels: int, out_channels: int, **kwargs) -> MessagePassing:
        mlp = MLP([in_channels, out_channels, out_channels], act=self.act, act_first=self.act_first, norm=None)
...
class GAT(BasicGNN):
    ...
    def init_conv(self, in_channels: T.Union[int, T.Tuple[int, int]], out_channels: int, **kwargs) -> MessagePassing:
        ...
        return Conv(in_channels, out_channels, heads=heads, concat=concat, dropout=self.dropout, **kwargs)
...
class GTConv(MessagePassing):  # pylint: disable=abstract-method
    ...
    def forward(self, x: Tensor, edge_index: Adj):
        ...
        x = self.propagate(edge_index, query=query, key=key, value=value, size=None)
        x = x.view(-1, H * C)
